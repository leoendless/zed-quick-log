...
var LOG_LABEL_PREFIX = "\u{1F50D} ";
connection.onInitialize(() => ({
  capabilities: {
    textDocumentSync: import_node.TextDocumentSyncKind.Incremental,
    codeActionProvider: {
      codeActionKinds: [import_node.CodeActionKind.Empty, import_node.CodeActionKind.RefactorRewrite],
...
function resolveTarget(doc, range) {
...
function buildInsertLogAction(doc, range) {
  ...
  const insertPos = import_node.Position.create(anchorLine, lineText.length);
  const edit = {
...
function buildDocumentScopedActions(doc) {
  ...
  actions.push({
    ...
    edit: {
      changes: {
        [doc.uri]: matchingLines.filter((i) => !/^\s*\/\//.test(lines[i])).map((i) => commentLineEdit(lines[i], i))
      }
    }
  });
  actions.push({
    ...
    edit: {
      changes: {
        [doc.uri]: matchingLines.filter((i) => /^\s*\/\/\s*console\.log\(/.test(lines[i])).map((i) => uncommentLineEdit(lines[i], i))
      }
    }
  });
  actions.push({
...
function commentLineEdit(line, lineNumber) {
  const leading = line.match(/^\s*/)?.[0] ?? "";
  const rest = line.slice(leading.length);
  return {
    range: {
      start: import_node.Position.create(lineNumber, 0),
      end: import_node.Position.create(lineNumber, line.length)
    },
    newText: `${leading}// ${rest}`
  };
}
function uncommentLineEdit(line, lineNumber) {
...
function deleteLineEdit(lines, lineNumber) {
  const isLast = lineNumber === lines.length - 1;
  return {
    range: {
      start: import_node.Position.create(lineNumber, 0),
      end: isLast ? import_node.Position.create(lineNumber, lines[lineNumber].length) : import_node.Position.create(lineNumber + 1, 0)
    },
    newText: ""
  };
}
function getLineText(doc, line) {
  return doc.getText({
    start: import_node.Position.create(line, 0),
    end: import_node.Position.create(line + 1, 0)
  }).replace(/\r?\n$/, "");
}
...
function findEnclosingContext(doc, line) {
  const patterns = [
...
function isLanguageKeyword(name) {
  return [
    "if",
    "else",
    "for",
